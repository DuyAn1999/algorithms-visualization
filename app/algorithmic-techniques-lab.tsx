...
export function AlgorithmicTechniquesLab() {
  ...
  useEffect(() => {
    const restored = parseCompletedLessons(readLocalValue("algolab-technique-progress"), lessonOrder);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => setCompleted((previous) => {
      ...
      const next = [...new Set([...parseCompletedLessons(readLocalValue("algolab-technique-progress"), lessonOrder), ...previous, lessonId])];
      writeLocalValue("algolab-technique-progress", JSON.stringify(next));
      return next;
    }));
