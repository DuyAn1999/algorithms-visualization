...
export function GraphFoundationsLab() {
  ...
  useEffect(() => {
    const restored = parseCompletedLessons(readLocalValue("algolab-graph-progress"), lessonOrder);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => setCompleted((previous) => {
      ...
      const next = [...new Set([...parseCompletedLessons(readLocalValue("algolab-graph-progress"), lessonOrder), ...previous, lessonId])];
      writeLocalValue("algolab-graph-progress", JSON.stringify(next));
      return next;
    }));
